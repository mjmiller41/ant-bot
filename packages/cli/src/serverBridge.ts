 ...
 * @antbot/server eagerly, and locating that package cannot itself depend on loading it.
 */
...
/**
 * Where @antbot/server's dependencies can be resolved from. Three layouts, in order:
 *
 * 1. A real resolve of the package — correct under npm, pnpm and yarn regardless of how the
 *    installer chose to hoist, which the old hardcoded `cli/node_modules/@antbot/server` symlink
 ...
 *    @antbot/server: the CLI and the daemon are one package, and better-sqlite3/playwright are
 *    its own dependencies.
 */
export function resolveServerPackageDir(deps: ResolveDeps): string | null {
  try {
    return deps.realpath(path.dirname(deps.resolve('@antbot/server/package.json')));
  } catch { /* not a separate package here; try the other layouts */ }

  const pkg = packageRootOf(deps.here, deps.exists);
  if (pkg) {
    const link = path.join(pkg, 'node_modules', '@antbot', 'server');
    if (deps.exists(link)) {
      try {
 ...
 *
 * The published build has no separate @antbot/server — the CLI and the daemon are bundled into
 ...
 * @antbot/server's package.json "exports" main entry may not resolve to a build artifact at all
 * times, so a literal `import('@antbot/server')` would make this file's typecheck depend on that
...
export async function startAntbotServer(opts: StartServerOptions): Promise<ServerInstance> {
  const mod = (await importFirst(candidates('@antbot/server', 'server.js'), 'the ant-bot daemon')) as {
