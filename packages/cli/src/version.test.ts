...
describe('one source of version truth', () => {
  ...
  it.each(['shared', 'server', 'cli', 'web'])('keeps packages/%s in lockstep with the root', (pkg) => {
    expect(readPkg(path.join(repoRoot, 'packages', pkg, 'package.json')).version).toBe(rootVersion);
  });
