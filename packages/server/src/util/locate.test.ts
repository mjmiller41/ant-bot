...
const CHECKOUT = {
  serverPkg: '/repo/packages/server/package.json',
  here: '/repo/packages/server/dist/api',
  webIndex: '/repo/packages/web/dist/index.html',
  skills: '/repo/skills',
};
const INSTALLED = {
...
describe('findPackageRoot', () => {
  it('finds the nearest package.json above a compiled module', () => {
    expect(findPackageRoot('/repo/packages/server/dist/api', fsOf([CHECKOUT.serverPkg])))
      .toBe(path.resolve('/repo/packages/server'));
  });
...
describe('findWebDist', () => {
  it('finds the sibling workspace package in a checkout', () => {
    ...
    expect(findWebDist(d)).toBe(path.resolve('/repo/packages/web/dist'));
  });
  ...
  it('prefers an explicitly resolvable @antbot/web over any layout guess', () => {
    const d = deps({
      here: CHECKOUT.here,
      resolve: (s) => (s === '@antbot/web/package.json' ? '/elsewhere/web/package.json' : null),
